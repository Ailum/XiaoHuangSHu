package com.lhj.framework.common.util;
...
public final class ParamUtils {
 private ParamUtils() {
     ...
     * 小哈书 ID 校验
     *
     * @param xiaohuangshuId
     * @return
     */
    public static boolean checkXiaohashuId(String xiaohuangshuId) {
        // 检查长度
        if (xiaohuangshuId.length() < ID_MIN_LENGTH || xiaohuangshuId.length() > ID_MAX_LENGTH) {
            return false;
        }
        // 检查格式
        Pattern pattern = Pattern.compile(ID_REGEX);
        return pattern.matcher(xiaohuangshuId).matches();
    }
